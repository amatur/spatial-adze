...
parser.add_argument("-ad","--all_diploid", help="all individual sampling", action = "store_true")
...
def all_diploid(nts, SEED):
    """
    Selects all individuals (instead of random sampling) and extracts diploid nodes.
    """
    print("Selecting all diploid individuals")
    
    # Get all individual indices
    inds = np.arange(nts.num_individuals)  # Select all individuals instead of random sampling
    
    samps = []
    for i in inds:
        samps.extend(nts.individual(i).nodes)  # Get all nodes from all individuals
    
    inds_out = [nts.node(i).individual for i in samps]
    assert set(inds) == set(inds_out)  # Ensure all individuals are included
    
    outfile = DPREFIX + "_" + OUTSUFFIX + "_all"
...
if(args.all_diploid):
	samps, outfile = all_diploid(nts, SEED)
	write_output(samps, outfile)
